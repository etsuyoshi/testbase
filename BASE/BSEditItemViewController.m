...

{
    ...
    if (session_id != NULL && !isLoaded) {
        [SVProgressHUD showWithStatus:@"商品情報を取得中" maskType:SVProgressHUDMaskTypeClear];
        NSString *url = [NSString stringWithFormat:@"%@/items/get_item?session_id=%@&item_id=%@",apiUrl,session_id, itemId];
        ...
        for ( int n = 0; n < 5; n++) {
            ...
            AFImageRequestOperation *operation = [AFImageRequestOperation
                                                  ...
                                                  success:^(NSURLRequest *request, NSHTTPURLResponse *response, UIImage *image) {
                                                      ...
                                                      //[getImageArray replaceObjectAtIndex:n withObject:image];
                                                      // assign image to myButton
                                                      switch (n) {
                                                          case 0:
                                                              [itemView.leftButton setImage:image forState:UIControlStateNormal];
                                                              ...
                                                              break;
                                                          case 1:
                                                              [itemView.centerLeftButton setImage:image forState:UIControlStateNormal];
                                                              centerLeftButtonImage = image;

                                                              itemView.centerButton.hidden = NO;
                                                              break;
                                                          case 2:
                                                              [itemView.centerButton setImage:image forState:UIControlStateNormal];
                                                              centerButtonImage = image;

                                                              itemView.centerRightButton.hidden = NO;
                                                              break;
                                                          case 3:
                                                              [itemView.centerRightButton setImage:image forState:UIControlStateNormal];
                                                              centerRightButtonImage = image;

                                                              itemView.rightButton.hidden = NO;
                                                              break;
                                                          case 4:
                                                              [itemView.rightButton setImage:image forState:UIControlStateNormal];
...
- (IBAction)submitItem:(id)sender
{
    ...
    //NSString *url = [NSString stringWithFormat:@"%@/items/edit_item?session_id=%@&item_id=%@&title=%@&price=%@&detail=%@&inc_variation=%d&visible=%d",apiUrl,session_id, itemId, itemName.text, price.text, detail.text, incVariation1, visible];
    NSString *url = [NSString stringWithFormat:@"%@",apiUrl];
    NSLog(@"入っているセッションid%@",session_id);
    NSLog(@"geturl:%@",url);
    //NSString *path = [NSString stringWithFormat:@"session_id=%@&item_id=%@&title=%@&price=%@&detail=%@&stock=%@&inc_variation=%d&visible=%d",session_id, itemId, itemName.text, price.text, detail.text, stockLabel1.text,incVariation1, visible];
    NSDictionary *parameters = @{
                                @"session_id":[BSTutorialViewController sessions],
    ...
    NSLog(@"geturlrequest:%@",request);

    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        
        NSLog(@"items/edit_item: %@",JSON);
        if ([JSON objectForKey:@"result"]) {
            [SVProgressHUD dismiss];

            [self uploadImage:[JSON valueForKey:@"result.Item"]];
        }
...
- (BOOL)uploadImage:(NSDictionary*)resultDict
{
    ...
    for (int n = 0; n < 5; n++) {
        ...
        isFinished = NO;
        NSMutableURLRequest *request = [client multipartFormRequestWithMethod:@"POST" path:@"items/add_image" parameters:parameters constructingBodyWithBlock: ^(id <AFMultipartFormData>formData) {
        ...
        [operation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject)
         {
             ...
             NSLog(@"画像アップロード実装: %@",jsons);
             NSLog(@"response: %@",[[jsons valueForKeyPath:@"error.validations.Item.image_file"] objectAtIndex:0]);
...
- (void)deleteItem{
    ...
    NSString *url = [NSString stringWithFormat:@"%@/items/delete_item?session_id=%@&item_id=%@",apiUrl, sessionId, itemId];
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        NSLog(@"削除商法: %@", JSON);
        NSString *errorString = [JSON valueForKeyPath:@"error"];
