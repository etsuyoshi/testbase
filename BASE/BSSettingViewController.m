...
- (void)setShopInfo
{
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        ...
        presentPaymentParam[@"bt_payment"] = [JSON valueForKeyPath:@"result.Shop.bt_payment"];
        presentPaymentParam[@"creditcart_payment"] = [JSON valueForKeyPath:@"result.Shop.creditcart_payment"];
        presentPaymentParam[@"cod_payment"] = [JSON valueForKeyPath:@"result.Shop.cod_payment"];
        if ([[JSON valueForKeyPath:@"result.Shop.cod_payment"] intValue] == 1) {
            presentPaymentParam[@"charge"] = [JSON valueForKeyPath:@"result.Shop.charge"];
        }
        if ([[JSON valueForKeyPath:@"result.Shop.cod_payment"] intValue] == 1) {
            presentPaymentParam[@"bank_name"] = [JSON valueForKeyPath:@"result.Shop.bank_name"];
            presentPaymentParam[@"branch_name"] = [JSON valueForKeyPath:@"result.Shop.branch_name"];
            presentPaymentParam[@"account_type"] = [JSON valueForKeyPath:@"result.Shop.account_type"];
            presentPaymentParam[@"account_name"] = [JSON valueForKeyPath:@"result.Shop.account_name"];
            presentPaymentParam[@"account_number"] = [JSON valueForKeyPath:@"result.Shop.account_number"];
        ...
        //特商法
        NSString *urlString = [NSString stringWithFormat:@"%@/users/get_transaction?session_id=%@",apiUrl,sessionId];
        NSLog(@"おせてます%@",urlString);
        urlString = [urlString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        ...
        AFHTTPClient *httpClient = [[AFHTTPClient alloc] initWithBaseURL:url1];
        //NSURLRequest *request = [NSURLRequest requestWithURL:url1];
        NSMutableURLRequest *request2 = [httpClient requestWithMethod:@"GET"
                                                                 path:@""
                                                           parameters:nil];
        ...
        AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request2 success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
            ...
            lastNameField.text = [JSON valueForKeyPath:@"result.Shop.last_name"];
            if ([[JSON valueForKeyPath:@"result.Shop.last_name"] isEqual:[NSNull null]])
                lastNameField.text = @"";
            NSLog(@"ここまではオケ%@",lastNameField.text);
            
            if ([[JSON valueForKeyPath:@"result.Shop.first_name"] isEqual:[NSNull null]]){
                firstNameField.text = @"";
            }else{
                firstNameField.text = [JSON valueForKeyPath:@"result.Shop.first_name"];
            }
            
            
            if ([[JSON valueForKeyPath:@"result.Shop.zip_code"] isEqual:[NSNull null]]){
                zipCodeField.text = @"";
            }else{
                zipCodeField.text = [JSON valueForKeyPath:@"result.Shop.zip_code"];
            }
            NSLog(@"ここまではオケzipCodeField.text%@",zipCodeField.text);
            
            if ([[JSON valueForKeyPath:@"result.Shop.prefecture"] isEqual:[NSNull null]]){
                prefectureLbl.text = @"";
                
            }else{
                prefectureLbl.text = [JSON valueForKeyPath:@"result.Shop.prefecture"];
                
            }
            NSLog(@"ここまではオケ prefectureLbl.text%@",zipCodeField.text);
            
            if ([[JSON valueForKeyPath:@"result.Shop.address"] isEqual:[NSNull null]]){
                addressTextView.text = @"住所（建物まで明記）";
                
            }else{
                addressTextView.text = [JSON valueForKeyPath:@"result.Shop.address"];
                
            }
            NSLog(@"ここまではオケ addressTextView.textt%@",addressTextView.text);
            
            if ([[JSON valueForKeyPath:@"result.Shop.tel_no"] isEqual:[NSNull null]]){
                telField.text = @"";
                
            }else{
                telField.text = [JSON valueForKeyPath:@"result.Shop.tel_no"];
                
            }
            NSLog(@"ここまではオケ telField.textt%@",telField.text);
            
            if ([[JSON valueForKeyPath:@"result.Shop.contact"] isEqual:[NSNull null]]){
                alterAddressTextView.text = @"";
                
            }else{
                alterAddressTextView.text = [JSON valueForKeyPath:@"result.Shop.contact"];
                
            }
            NSLog(@"ここまではオケalterAddressTextView.texttt%@",alterAddressTextView.text);
            
            if ([[JSON valueForKeyPath:@"result.Shop.price"] isEqual:[NSNull null]]){
                priceTextView.text = @"";
                
            }else{
                priceTextView.text = [JSON valueForKeyPath:@"result.Shop.price"];
                
            }
            NSLog(@"ここまではオケapriceTextView.text.texttt%@",priceTextView.text);
            
            if ([[JSON valueForKeyPath:@"result.Shop.pay"] isEqual:[NSNull null]]){
                payTextView.text = @"";
                
            }else{
                payTextView.text = [JSON valueForKeyPath:@"result.Shop.pay"];
                
            }
            
            if ([[JSON valueForKeyPath:@"result.Shop.service"] isEqual:[NSNull null]]){
                serviceTextView.text = @"";
                
            }else{
                serviceTextView.text = [JSON valueForKeyPath:@"result.Shop.service"];
            }
            if ([[JSON valueForKeyPath:@"result.Shop.return"] isEqual:[NSNull null]]){
                returnTextVfiew.text = @"";
                
            }else{
                returnTextVfiew.text = [JSON valueForKeyPath:@"result.Shop.return"];
...
- (void)viewDidLoad
{
    ...
    //グーグルアナリティクス
    self.trackedViewName = @"shopSetting";
...
-(void)saveShopInfo:(id)sender{
    if ([sender tag] == 2) {
        ...
        NSString *urlString = [NSString stringWithFormat:@"%@/users/edit_transaction?session_id=%@&last_name=%@&first_name=%@&zip_code=%@&prefecture=%@&address=%@&tel_no=%@&contact=%@&price=%@&pay=%@&service=%@&return=%@",apiUrl,sessionId,lastNameField.text,firstNameField.text,zipCodeField.text,prefectureLbl.text,addressTextView.text,telField.text,alterAddressTextView.text,priceTextView.text,payTextView.text,serviceTextView.text,returnTextVfiew.text];
        NSLog(@"おせてます%@",urlString);
        urlString = [urlString stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        ...
        AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
            NSLog(@"特商法！: %@", JSON);
            NSArray *telArray = [JSON valueForKeyPath:@"error.validations.User.tel_no"];
            if (telArray.count) {
            ...
            NSArray *zipcodeArray = [JSON valueForKeyPath:@"error.validations.User.zip_code"];
        ...
        if (visible) {
            shopStatus = @"&able_to_business=1";
        }else{
            shopStatus = @"&able_to_business=0";
        }
        
        //決済方法変更は単独でapiが作られたので以下のコードは必要ない
        /*
        if (importPayment2) {
            NSString *creditParam = [NSString string];
            if ([[importPayment2 objectForKey:@"credit"] intValue] == 1) {
                creditParam = [NSString stringWithFormat:@"&creditcart_payment=1"];
            }else{
                creditParam = [NSString stringWithFormat:@"&creditcart_payment=0"];
            }
            
            NSMutableString *codParam = [NSMutableString string];
            NSLog(@"cod:%d",[[importPayment2 objectForKey:@"cod"] intValue]);
            if ([[importPayment2 objectForKey:@"cod"] intValue] == 1) {
                codParam = [NSMutableString stringWithFormat:@"&cod_payment=1"];
                [codParam appendString:[NSMutableString stringWithFormat:@"&charge=%@",[importPayment2 objectForKey:@"charge"]]];
            }else{
                codParam = [NSMutableString stringWithFormat:@"&cod_payment=0"];
            }
            
            
            
            
            NSMutableString *bankParam =  [NSMutableString string];
            if ([[importPayment2 objectForKey:@"bank"] intValue] == 1) {
                bankParam = [NSMutableString stringWithFormat:@"&bt_payment=1"];
                [bankParam appendString:[NSString stringWithFormat:@"&bank_name=%@",[importPayment2 objectForKey:@"bankName"]]];
                [bankParam appendString:[NSString stringWithFormat:@"&branch_name=%@",[importPayment2 objectForKey:@"branchName"]]];
                [bankParam appendString:[NSString stringWithFormat:@"&account_type=%@",[importPayment2 objectForKey:@"accountVary"]]];
                [bankParam appendString:[NSString stringWithFormat:@"&account_name=%@",[importPayment2 objectForKey:@"account_name"]]];
                [bankParam appendString:[NSString stringWithFormat:@"&account_number=%@",[importPayment2 objectForKey:@"accountNumber"]]];
            }else{
                bankParam = [NSMutableString stringWithFormat:@"&bt_payment=0"];
            }
            NSMutableString *deliveryParam = [NSMutableString string];
            if (isUsed) {
                deliveryParam = [NSMutableString stringWithFormat:@"&delivery=1"];
                [deliveryParam appendString:[NSMutableString stringWithFormat:@"&fee=%@",deliveryField.text]];
            }else{
                deliveryParam = [NSMutableString stringWithFormat:@"&delivery=0"];
            }
            ...
            [urlString appendString:descriptionParam];
            [urlString appendString:deliveryParam];
            [urlString appendString:shopStatus];
            [urlString appendString:creditParam];
            [urlString appendString:codParam];
            [urlString appendString:[bankParam stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding]];
        }else{
            
            
            NSString *creditParam = [NSString string];
            if ([[presentPaymentParam objectForKey:@"creditcart_payment"] intValue] == 1) {
                creditParam = [NSString stringWithFormat:@"&creditcart_payment=1"];
            }else{
                creditParam = [NSString stringWithFormat:@"&creditcart_payment=0"];
            }
            ...
            NSMutableString *codParam = [NSMutableString string];
            if ([[presentPaymentParam objectForKey:@"cod_payment"] intValue] == 1) {
                codParam = [NSMutableString stringWithFormat:@"&cod_payment=1"];
                [codParam appendString:[NSMutableString stringWithFormat:@"&charge=%@",[presentPaymentParam objectForKey:@"charge"]]];
            }else{
                codParam = [NSMutableString stringWithFormat:@"&cod_payment=0"];
            }
            ...
            NSMutableString *bankParam =  [NSMutableString string];
            if ([[presentPaymentParam objectForKey:@"bt_payment"] intValue] == 1) {
                bankParam = [NSMutableString stringWithFormat:@"&bt_payment=1"];
                [bankParam appendString:[NSString stringWithFormat:@"&bank_name=%@",[presentPaymentParam objectForKey:@"bank_name"]]];
                [bankParam appendString:[NSString stringWithFormat:@"&branch_name=%@",[presentPaymentParam objectForKey:@"branch_name"]]];
                [bankParam appendString:[NSString stringWithFormat:@"&account_type=%@",[presentPaymentParam objectForKey:@"account_type"]]];
                [bankParam appendString:[NSString stringWithFormat:@"&account_name=%@",[presentPaymentParam objectForKey:@"account_name"]]];
                [bankParam appendString:[NSString stringWithFormat:@"&account_number=%@",[presentPaymentParam objectForKey:@"account_number"]]];
            }else{
                bankParam = [NSMutableString stringWithFormat:@"&bt_payment=0"];
            }
        ...
        */
        ...
        AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
            NSLog(@"特商法！: %@", JSON);
            /*
            NSArray *errorAcountTypeArray = [JSON valueForKeyPath:@"error.validations.User.account_type"];
            if (errorAcountTypeArray.count) {
                NSString *errorAcountType = [errorAcountTypeArray objectAtIndex:0];
                [SVProgressHUD showErrorWithStatus:errorAcountType];
                
                return ;
            }
            NSArray *drawingsArray = [JSON valueForKeyPath:@"error.validations.User.drawings"];
            if (drawingsArray.count) {
                NSString *drawings = [drawingsArray objectAtIndex:0];
                [SVProgressHUD showErrorWithStatus:drawings];
                
                return ;
            }
            NSArray *bankNameArray = [JSON valueForKeyPath:@"error.validations.User.bank_name"];
            if (bankNameArray.count) {
                NSString *bankName = [bankNameArray objectAtIndex:0];
                [SVProgressHUD showErrorWithStatus:bankName];
                
                return ;
            }
            NSArray *branchNameArray = [JSON valueForKeyPath:@"error.validations.User.branch_name"];
            if (branchNameArray.count) {
                NSString *branchName = [branchNameArray objectAtIndex:0];
                [SVProgressHUD showErrorWithStatus:branchName];
                
                return ;
            }
            NSArray *accountNameArray = [JSON valueForKeyPath:@"error.validations.User.account_name"];
            if (accountNameArray.count) {
                NSString *accountName = [accountNameArray objectAtIndex:0];
                [SVProgressHUD showErrorWithStatus:accountName];
                
                return ;
            }
            NSArray *accountNumberArray = [JSON valueForKeyPath:@"error.validations.User.account_number"];
            if (accountNumberArray.count) {
                NSString *accountNumber = [accountNumberArray objectAtIndex:0];
            ...
            */
            ...
            NSArray *deliveryFeeArray = [JSON valueForKeyPath:@"error.validations.User.Delivery"];
