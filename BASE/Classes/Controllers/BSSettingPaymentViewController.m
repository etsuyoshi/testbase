...
- (void)viewDidLoad
{
    ...
    //グーグルアナリティクス
    self.screenName = @"settingPayment";
    ...
    */
    ...
    [[BSSellerAPIClient sharedClient] getUsersPaymentWithSessionId:[BSUserManager sharedManager].sessionId completion:^(NSDictionary *results, NSError *error) {
        
        
        NSLog(@"getUsersPaymentWithSessionId: %@", results);
        NSDictionary *presentPaymentInfo = [results valueForKeyPath:@"result.User"];
...
-(void)savePaymentInfo:(id)sender{
        ...
        if ([self validationCheck]) {
            ...
            [[BSSellerAPIClient sharedClient] postUsersEditPaymentWithSessionId:[BSUserManager sharedManager].sessionId creditcartPayment:[credit intValue] codPayment:[cod intValue] charge:[cashField.text intValue] btPayment:[bank intValue] bankName:bankNameField.text branchName:branchNameField.text accountType:accountVaryField.text accountName:accountHolderField.text accountNumber:[accountNumberField.text intValue] completion:^(NSDictionary *results, NSError *error) {
                
                NSLog(@"決済方法変更後: %@", results);
                if (error) {
