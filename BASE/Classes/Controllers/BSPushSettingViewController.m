...
- (void)viewDidLoad
{
    ...
    [[BSCommonAPIClient sharedClient] getPushNotificationsSettingWithSessionId:nil token:[AppDelegate getDeviceToken] curation:2 order:2 completion:^(NSDictionary *results, NSError *error) {
        ...
        NSLog(@"トークンの取得: %@", results);
        
        NSString *curation = [results valueForKeyPath:@"result.PushNotification.curation"];
        NSString *order = [results valueForKeyPath:@"result.PushNotification.order"];
...
- (void)switchcontroll:(id)sender{
    ...
    [[BSCommonAPIClient sharedClient] getPushNotificationsSettingWithSessionId:nil token:[AppDelegate getDeviceToken] curation:curation order:order completion:^(NSDictionary *results, NSError *error) {
