...
- (void)login:(id)sender{
    ...
    [tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"BSTutorialViewController"
                                                          action:@"login"
                                                           label:nil
                                                           value:nil] build]];
    ...
    [[BSSellerAPIClient sharedClient] postUsersSignInWithMailAddress:email.text password:password.text completion:^(NSDictionary *results, NSError *error) {
        ...
        if (!Auth[@"session_id"]){
            ...
            [[BSCommonAPIClient sharedClient] getPushNotificationsSettingWithSessionId:session token:[AppDelegate getDeviceToken] curation:2 order:2 completion:^(NSDictionary *results, NSError *error) {
...
- (void)signUp:(id)sender{
    ...
    */
    ...
    [[BSSellerAPIClient sharedClient] postUsersSignUpWithShopId:signUpShopName.text password:signUpPassword.text mailAddress:signUpemail.text completion:^(NSDictionary *results, NSError *error) {
        ...
        NSLog(@"ログイン情報！: %@", results);
        NSArray *shopIdErrorArray = [results valueForKeyPath:@"error.validations.User.shop_id"];
        NSArray *passwordErrorArray = [results valueForKeyPath:@"error.validations.User.pre_password"];
        NSArray *emailErrorArray = [results valueForKeyPath:@"error.validations.User.mail_address"];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if (buttonIndex == 1) {
        ...
        [[BSSellerAPIClient sharedClient] postUsersResetPasswordWithMailAddress:[[alertView textFieldAtIndex:0] text] completion:^(NSDictionary *results, NSError *error) {
            
            
            NSLog(@"postUsersResetPasswordWithMailAddress: %@", results);
            NSDictionary *errorDictionary = [results valueForKey:@"error"];
            if (errorDictionary) {
                [SVProgressHUD showErrorWithStatus:@"入力が正しくありません"];
                NSString *errorMessage = [errorDictionary valueForKey:@"message"];
