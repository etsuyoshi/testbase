...
-(void)sendQuestion:(id)sender{
    ...
    [[BSSellerAPIClient sharedClient] getInquirysWithSessionId:[BSUserManager sharedManager].sessionId title:[questionLabel.text stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] name:[userNameField.text stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] mailAddress:escapedString inquiry:[questionTextView.text stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding] completion:^(NSDictionary *results, NSError *error) {
        
        
        NSLog(@"getInquirysWithSessionId: %@", results);
        
        
        NSArray *nameArray = [results valueForKeyPath:@"error.validations.Inquiry.name"];
        if (nameArray.count) {
            NSString *name = nameArray[0];
            [SVProgressHUD showErrorWithStatus:name];
            return ;
        }
        NSArray *mailArray = [results valueForKeyPath:@"error.validations.Inquiry.mail_address"];
        if (mailArray.count) {
            NSString *mail = mailArray[0];
            [SVProgressHUD showErrorWithStatus:mail];
            return ;
        }
        NSArray *inquiryArray = [results valueForKeyPath:@"error.validations.Inquiry.inquiry"];
