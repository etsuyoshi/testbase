...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    //グーグルアナリティクス
    self.screenName = [NSString stringWithFormat:@"aboutItem,itemId:%@",itemId];
    
    
    [[BSBuyerAPIClient sharedClient] getShopItemWithItemId:itemId deviseName:nil deviseOS:nil deviseId:nil completion:^(NSDictionary *results, NSError *error) {
       
        [loadingImageView stopAnimating];
        NSLog(@"getItemWithSessionId: %@", results);
        NSDictionary *result = results[@"result"];
        NSString *imageUrl = results[@"image_url"];
        importShopId = [results valueForKeyPath:@"result.Item.user_id"];
        itemUrl = results[@"item_url"];
        ...
        socialInfo[@"itemUrl"] = [results valueForKeyPath:@"item_url"];
        if (result) {
            if ([BSDefaultViewObject isMoreIos7]) {
    ...
    if (favorited) {
        [UIView animateWithDuration:0.2 animations:^{
            ...
            [UIView animateWithDuration:0.2 animations:^{
                ...
                [tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"BSAboutItemViewController"
                                                                      action:[NSString stringWithFormat:@"itemId:%@,アイテム名:%@",itemId,itemName]
                                                                       label:nil
                                                                       value:nil] build]];
            }];
...
-(void)addIos7SubViews:(NSDictionary*)jsonDictionary getImageUrl:(NSString*)imageUrl {
    ...
    [requestOperation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        UIImage *getFirstImage = responseObject;
        ...
        for (int a = 0; a < imageNameArray.count; a++) {
            ...
            [requestOperation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
                ...
                NSLog(@"Response: %@", responseObject);
                [otherItemImageView setImage:responseObject];
...
- (void)addToCartIos7:(id)sender{
    ...
    id<GAITracker> tracker = [[GAI sharedInstance] defaultTracker];
    [tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"BSAboutItemViewController"
                                                          action:[NSString stringWithFormat:@"addToCart itemId:%@shopId:%@",itemId,importShopId]
                                                           label:nil
                                                           value:nil] build]];
    //お気に入りに追加
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
