...
- (void)viewDidLoad
{
    ...
    //グーグルアナリティクス
    self.screenName = [NSString stringWithFormat:@"buyShop,shopId:%@",analyShopId];
    ...
    */
    ...
    [[BSBuyerAPIClient sharedClient] getShopWithUserId:userId deviseName:deviceName deviseOS:[NSString stringWithFormat:@"%f",iOSVersion] deviseId:deviceId completion:^(NSDictionary *results, NSError *error) {
       
        
        id JSON = results;
        NSLog(@"getShopWithUserId: %@", JSON);
        ...
        [tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"BSShopViewController"
                                                              action:[NSString stringWithFormat:@"%@,ショップ名:%@",[BSBuyTopViewController getSelectedShopInfo],label.text]
                                                               label:nil
                                                               value:nil] build]];
        ...
        if ([logoTitle isEqual:[NSNull null]] || [logoTitle isEqualToString:@""]) {
            ...
            [[BSBuyerAPIClient sharedClient] getShopItemsWithUserId:userId deviseName:nil deviseOS:nil deviseId:nil completion:^(NSDictionary *results, NSError *error) {
                
                NSLog(@"ショップ情報！: %@", results);
                smallMenuView.hidden = NO;
                ItemArray = [results valueForKeyPath:@"result"];
                imageRootUrl = [results valueForKeyPath:@"image_url"];
                
                socialInfo = [NSMutableDictionary dictionary];
                socialInfo[@"facebook"] = [results valueForKeyPath:@"social.facebook_content"];
                socialInfo[@"twitter"] = [results valueForKeyPath:@"social.tweet_content"];
                socialInfo[@"line"] = [results valueForKeyPath:@"social.line_content"];
                socialInfo[@"shopUrl"] = [results valueForKeyPath:@"shop_url"];
        ...
        requestOperation.responseSerializer.acceptableContentTypes = [NSSet setWithObject:@"image/jpeg"];
         */
        [requestOperation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
            ...
            [[BSBuyerAPIClient sharedClient] getShopItemsWithUserId:userId deviseName:nil deviseOS:nil deviseId:nil completion:^(NSDictionary *results, NSError *error) {
                ...
                NSLog(@"[JSON valueForKeyPath:social.facebook_content]]%@",[results valueForKeyPath:@"social.facebook_content"]);
                socialInfo = [NSMutableDictionary dictionary];
                socialInfo[@"facebook"] = [results valueForKeyPath:@"social.facebook_content"];
                socialInfo[@"twitter"] = [results valueForKeyPath:@"social.tweet_content"];
                socialInfo[@"line"] = [results valueForKeyPath:@"social.line_content"];
...
- (void)checkFan{
    ...
    [[BSBuyerAPIClient sharedClient] getCheckFollowShopWithApiTokenId:[AppDelegate getTokenId] uniqueKey:uniqueKey userId:userId completion:^(NSDictionary *results, NSError *error) {
       
        NSLog(@"getFollowShopWithApiTokenId:%@",results);
...
- (void)beFanOfThisShop:(id)sender{
    ...
    [[BSBuyerAPIClient sharedClient] getFollowShopWithApiTokenId:[AppDelegate getTokenId] uniqueKey:uniqueKey userId:userId completion:^(NSDictionary *results, NSError *error) {
        
        
        NSLog(@"ファンになったよ！%@",results);
...
- (void)beUnfanOfThisShop:(id)sender{
    ...
    [[BSBuyerAPIClient sharedClient] getUnfollowShopWithApiTokenId:[AppDelegate getTokenId] uniqueKey:uniqueKey userId:userId completion:^(NSDictionary *results, NSError *error) {
       
        
        NSLog(@"getUnfollowShopWithApiTokenId:%@",results);
        ...
        if ([logoTitle isEqual:[NSNull null]] || [logoTitle isEqualToString:@""]){
            if (indexPath.section == 0) {
                ...
                if ([imageUrl1 isEqual:[NSNull null]] || [imageUrl1 isEqualToString:@""]) {
                    ...
                    AFHTTPRequestOperation *requestOperation = [[AFHTTPRequestOperation alloc] initWithRequest:getImageRequest1];
                    requestOperation.responseSerializer = [AFImageResponseSerializer serializer];
                    [requestOperation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
                        NSLog(@"Response: %@", responseObject);
                ...
                if ([imageUrl2 isEqual:[NSNull null]] || [imageUrl2 isEqualToString:@""]) {
                    ...
                    AFHTTPRequestOperation *requestOperation = [[AFHTTPRequestOperation alloc] initWithRequest:getImageRequest2];
                    requestOperation.responseSerializer = [AFImageResponseSerializer serializer];
                    [requestOperation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
                        NSLog(@"Response: %@", responseObject);
...
- (void)updateItemCell:(ItemTableViewCell *)cell atIndexPath:(NSIndexPath *)indexPath {
    ...
    requestOperation = [[AFHTTPRequestOperation alloc] initWithRequest:getImageRequest2];
    requestOperation.responseSerializer = [AFImageResponseSerializer serializer];
    [requestOperation setCompletionBlockWithSuccess:^(AFHTTPRequestOperation *operation, id responseObject) {
        NSLog(@"Response: %@", responseObject);
...
- (void)showCartIOS7:(id)sender{
    ...
    [manager POST:[url absoluteString] parameters:mdic success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        if (errormessage) {
            ...
            [self setCartViewIOS7:responseObject];
        }
