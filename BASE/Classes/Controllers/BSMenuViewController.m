    ...
    if (indexPath.section==0) {
        ...
        if (indexPath.row == 4){
            ...
            if (session_id) {
                ...
                [[BSSellerAPIClient sharedClient] getUsersSignOutWithSessionId:[BSUserManager sharedManager].sessionId token:[AppDelegate getDeviceToken] completion:^(NSDictionary *results, NSError *error) {
                    
                    NSLog(@"getUsersSignOutWithSessionId: %@", results);
                    NSString *errorMessage = [results valueForKeyPath:@"error.message"];
                    NSLog(@"エラーログ%@",errorMessage);
