...
- (IBAction)submitPhoto:(id)sender
{
    ...
    [tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"BSAddItemViewController"
                                                          action:@"addItem"
                                                           label:nil
                                                           value:nil] build]];
    ...
    [manager GET:[url absoluteString]
      parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
          ...
          NSLog(@"注文詳細！: %@", responseObject);
          userAgent = (operation.request.allHTTPHeaderFields)[@"User-Agent"];
          
          NSLog(@"jsonデータ:%@",responseObject);
          jsonItemId = [responseObject valueForKeyPath:@"result.Item.id"];
          NSLog(@"jsonデータのその2%@",responseObject);
          
          NSArray *error = [responseObject valueForKeyPath:@"error.validations.Item.Variation"];
          ...
          if (jsonItemId){
              ...
              // add image data
              NSData *imageData =  [[NSData alloc] initWithData:UIImageJPEGRepresentation(takePicture1, 1.0)];
              if (imageData) {
                  [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
    ...
    if (upTimes <= uploadImageArray.count) {
        if (jsonItemId) {
            ...
            if (upTimes == 2) {
                ...
                imageData = [[NSData alloc] initWithData:UIImageJPEGRepresentation(takePicture2,1.0)];
            }else if (upTimes == 3) {
                //imageData = UIImageJPEGRepresentation(takePicture3, 1.0);
                imageData = [[NSData alloc] initWithData:UIImageJPEGRepresentation(takePicture3,1.0)];

            }else if (upTimes == 4) {
                //imageData = UIImageJPEGRepresentation(takePicture4, 1.0);
                imageData = [[NSData alloc] initWithData:UIImageJPEGRepresentation(takePicture4,1.0)];

            }else if (upTimes == 5) {
                //imageData = UIImageJPEGRepresentation(takePicture5, 1.0);
                imageData = [[NSData alloc] initWithData:UIImageJPEGRepresentation(takePicture5,1.0)];
            }
            if (imageData) {
                [body appendData:[[NSString stringWithFormat:@"--%@\r\n", boundary] dataUsingEncoding:NSUTF8StringEncoding]];
                [body appendData:[[NSString stringWithFormat:@"Content-Disposition: form-data; name=\"image_file\"; filename=\"image.jpg\"\r\n"] dataUsingEncoding:NSUTF8StringEncoding]];
