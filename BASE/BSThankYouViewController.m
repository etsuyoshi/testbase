...
- (void)viewDidLoad
{
    ...
    */
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        ...
        twitterContent = [JSON valueForKeyPath:@"result.tweet_content"];
        facebookContent = [JSON valueForKeyPath:@"result.facebook_content"];
        shopUrl = [JSON valueForKeyPath:@"shop_url"];
        boughtShopId = [JSON valueForKeyPath:@"result.id"];
        [self deleteCartItem];
        //ロゴがあるorない場合のビュー
        if ([logoTitle isEqual:[NSNull null]] || [logoTitle isEqualToString:@""]) {
            [self noLogoArrange:JSON];
        }else{
            [self existedLogoArrange:JSON];
        }
