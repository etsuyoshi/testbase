...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@/savings/get_savings?session_id=%@&=",apiUrl,sessionId];
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        NSLog(@"注文詳細！: %@", JSON);
        historyDetailArray = [JSON valueForKeyPath:@"result"];
...
- (void)viewDidLoad
{
    ...
    //グーグルアナリティクス
    self.trackedViewName = @"moneyHistory";
