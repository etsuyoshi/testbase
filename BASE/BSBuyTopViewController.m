...
- (void)viewDidLoad
{
    ...
    if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone){
        ...
        if ([BSDefaultViewObject isMoreIos7]) {
            ...
            [TWStatus showLoadingWithStatus:loadingString];
...
-(void)refreshHomeTable:(NSString*)tokenId pages:(int)pages{
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        ...
        addFollowShopInfo = [JSON valueForKeyPath:@"result"];
        //NSString *resultString  = [JSON valueForKeyPath:@"result"];
        maxPage = [JSON valueForKeyPath:@"maxPage"];
        [UIApplication sharedApplication].networkActivityIndicatorVisible = NO;
        if ([maxPage intValue] == 0 && pages > 1) {
            [refreshControl endRefreshing];
            maxPage = @"1";
            followItemImageName = [JSON valueForKeyPath:@"image_url"];
            logoUrl = [JSON valueForKeyPath:@"logo_url"];
        ...
        //NSLog(@"ショップ数！: %d", addFollowShopInfo.count);
        
        
        

        /*
        NSString *homeResult = [JSON valueForKey:@"result"];
        NSLog(@"ほーむの結果%@kkk",homeResult);
        if (![homeResult isEqualToString:@""]) {
            NSArray *homeShopArray = [JSON valueForKey:@"result"];
            NSLog(@"フォロー数%d",homeShopArray.count);
...
- (void)setShopTable{
    ...
    /*
     ...
     // Add both effects to your view
     [myFrontalView addMotionEffect:group];
     */
    //[homeTable reloadData];
...
- (void)setHomeTable{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@/push_notifications/settings?token=%@",apiUrl,[AppDelegate getDeviceToken]];
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        NSLog(@"トークンの取得二回目: %@", JSON);
       apnTokenId = [JSON valueForKeyPath:@"result.PushNotification.apn_token_id"];
       // NSLog(@"ぷっしゅのいｄ%@",tokenId);
        ...
        if ([BSDefaultViewObject isMoreIos7]) {
            ...
            [refreshControl addTarget:self
                               action:@selector(initializeHomeTable)
...
-(void)connectGetItems{
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        ...
        NSLog(@"キュレーション情報！: %@", JSON);
        //NSString *imageRootUrl = [JSON valueForKeyPath:@"image_url"];
        //NSDictionary *result = [JSON valueForKeyPath:@"result"];
        
        //商品画像のurlを取得
        s3Url = [JSON valueForKeyPath:@"image_url"];
        
        categoryArray = [JSON valueForKeyPath:@"result.Category"];
        //横スライドのメニューのカテゴリー一覧
        ...
        /*
         //ショップ一覧
         NSDictionary *shop = [category objectAtIndex:0];
         //ショップ詳細一覧
         shop2 = [shop valueForKeyPath:@"Shop"];
         NSLog(@"ショップ数%d",shop2.count);
         */
    } failure:^(NSURLRequest *request, NSHTTPURLResponse *response, NSError *error, id JSON){
        NSLog(@"Error: %@", error);
    }];
    ...
    //見えているビューにcscrollToTopを有効にする
        ...
        if (tableView.tag != 100 + categoryArray.count) {
            ...
            /*
             ...
             [shopBtn setBackgroundImage:resultImg forState:UIControlStateNormal];
             resultImg = nil;
             [UIView animateWithDuration:0.6f
             delay:0.0f
             options:UIViewAnimationOptionAllowUserInteraction
             ...
             NSLog(@"spモード");
             [shopBtn setBackgroundImage:getImage forState:UIControlStateNormal];
             [UIView animateWithDuration:0.6f
             delay:0.0f
             options:UIViewAnimationOptionAllowUserInteraction
             animations:^(void){
...
- (void)showCartIOS7:(id)sender{
    ...
    AFHTTPRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        ...
        if (errormessage) {
            ...
            [self arrangeFavoriteViewIOS7:JSON];
        }
    } failure:^(NSURLRequest *request, NSHTTPURLResponse *response, NSError *error, id JSON){
        NSLog(@"Error: %@", error);
    }];
