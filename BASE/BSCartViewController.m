...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSMutableURLRequest *request = [httpClient requestWithMethod:@"POST" path:@"" parameters:itemIdParams];
    AFHTTPRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        NSLog(@"カートアイテム情報: %@", JSON);
        
        

        
        BOOL arranged = [self arrangeShopInfo:JSON];
        imageRootUrl = JSON[@"image_url"];
    ...
    */
    ...
    if (indexPath.row == 0) {
        ...
        */
        ...
        NSLog(@"＠あらめーた！section:%d  orderItemDictionary: %@", indexPath.section,orderItemDictionary);
        NSMutableURLRequest *request = [httpClient requestWithMethod:@"POST" path:@"" parameters:orderItemDictionary];
        AFHTTPRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
            NSLog(@"注文内容の取得: %@", JSON);
            ...
            deliveryFee = [JSON valueForKeyPath:@"result.DeliveryFee.fee"];
            NSString *prefDelivery = [JSON valueForKeyPath:@"result.DeliveryFee.prefSetting"];
            ...
            totalprice = [[JSON valueForKeyPath:@"result.Cart.subtotal"] intValue];
...
- (void)purchase:(id)sender{
    ...
    NSLog(@"＠あらめーた！: %@", orderItemDictionary);
    NSMutableURLRequest *request = [httpClient requestWithMethod:@"POST" path:@"" parameters:orderItemDictionary];
    AFHTTPRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        NSLog(@"カートの中身の注文内容の取得: %@", JSON);
        NSDictionary *error = JSON[@"error"];
        NSString *errormessage = error[@"message"];
        NSString *errormessage1 = [JSON valueForKeyPath:@"result.Cart.1.error"];
        //NSString *deliveryFee = @"0";
        NSLog(@"えあっっっっｒ%@",errormessage1);
        NSLog(@"Error message: %@", errormessage);
        NSString *versionError = [JSON valueForKeyPath:@"error.versionError"];
        ...
        if (errormessage) {
            ...
            //アイテム情報
            NSLog(@"アイテム情報%@",shopInfo);
            //NSDictionary *shopsDictionary = [shopInfo objectAtIndex:theButton.section];
            //NSString *shopNameId = [shopsDictionary objectForKey:@"shopNameId"];
            //NSArray *itemsInfoArray = [shopsDictionary objectForKey:@"itemInfo"];
            ...
            //ポスト(カートに入ってるアイテム情報)するパラメータを作成
            orderItemDictionary = [NSMutableDictionary dictionary];
            [orderItemDictionary setObject:shopNameId forKey:@"shop_id"];
            
            
            int rows = 0;
            rows = rows + [[itemRowsArray objectAtIndex:theButton.section] intValue];
            NSString *incVariation;
            for (int n = 0; n < rows; n++) {
                NSDictionary *itemInfoDictionary = [itemsInfoArray objectAtIndex:n];
                incVariation = [itemInfoDictionary objectForKey:@"incVariation"];
                NSString *itemId = [itemInfoDictionary objectForKey:@"item"];
                if ([incVariation intValue] == 1) {
                    NSArray *variationArray = [itemInfoDictionary objectForKey:@"variation"];
                    for (int x = 0; x < cartArray.count; x++) {
                        NSLog(@"カートヴァリエーション%@",cartArray);
                        NSDictionary *cartItem = [cartArray objectAtIndex:x];
                        NSString *cartItemId = [cartItem objectForKey:@"itemId"];
                        NSLog(@"あいでぃ1%@そのに%@",itemId,cartItemId);
                        if ([itemId isEqualToString:cartItemId]) {
                            NSString *cartVariationName = [cartItem objectForKey:@"variationName"];
                            NSString *cartItemStock = [cartItem objectForKey:@"variationStock"];
                            
                            NSLog(@"カートのバリエージョン%@",cartVariationName);
                            for (int a = 0; a < variationArray.count ; a++) {
                                NSDictionary *itemDict = [variationArray objectAtIndex:a];
                                NSString *variationName = [itemDict objectForKey:@"variation"];
                                NSString *variationId = [itemDict objectForKey:@"id"];
                                if ([cartVariationName isEqualToString:variationName]) {
                                    [orderItemDictionary setObject:itemId forKey:[NSString stringWithFormat:@"item_id[%d]",n]];
                                    [orderItemDictionary setObject:variationId forKey:[NSString stringWithFormat:@"variation_id[%d]",n]];
                                    [orderItemDictionary setObject:cartItemStock forKey:[NSString stringWithFormat:@"amount[%d]",n]];
                                }
                            }
                        }
                    }
                    ...
                    [orderItemDictionary setObject:itemId forKey:[NSString stringWithFormat:@"item_id[%d]",n]];
                    NSString *stockString = [cartSelectedStockArray objectAtIndex: n];
                    [orderItemDictionary setObject:stockString forKey:[NSString stringWithFormat:@"amount[%d]",n]];
                }
            }
             */
