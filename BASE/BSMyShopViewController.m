...
- (void)viewDidLoad
{
    ...
    */
    ...
    NSString *urlString2 = [NSString stringWithFormat:@"%@/design/get_logo?session_id=%@&=",apiUrl,sessionId];
    NSURL *url = [NSURL URLWithString:urlString2];
    AFHTTPClient *httpClient2 = [[AFHTTPClient alloc] initWithBaseURL:url];
    //NSURLRequest *request = [NSURLRequest requestWithURL:url1];
    NSMutableURLRequest *request2 = [httpClient2 requestWithMethod:@"GET"
                                                            path:@""
                                                      parameters:nil];
    
    AFJSONRequestOperation *operation1 = [AFJSONRequestOperation JSONRequestOperationWithRequest:request2 success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        NSLog(@"注文詳細！: %@", JSON);
