...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@/order_headers/get_order?session_id=%@&unique_key=%@",apiUrl,sessionId,importId];
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        NSLog(@"注文詳細！: %@", JSON);
        ordersDict = [JSON valueForKeyPath:@"result"];
        orderItemsArray = [JSON valueForKeyPath:@"result.Order"];
...
- (void)viewDidLoad
{
    ...
    //グーグルアナリティクス
    self.trackedViewName = @"orderDetails";
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (status) {
        ...
        NSString *orderId = [ordersDict valueForKeyPath:@"Order.id"];
        
        NSString *sessionId = [BSTutorialViewController sessions];
        NSString *urlString = [NSString stringWithFormat:@"%@/orders/change_dispatch_status?session_id=%@&order_id=%d&status=%@",apiUrl,sessionId,orderItemId,status];
        NSURL *url = [NSURL URLWithString:urlString];
        NSLog(@"ステータス変更のget:%@",urlString);
        AFHTTPClient *httpClient = [[AFHTTPClient alloc] initWithBaseURL:url];
        //NSURLRequest *request = [NSURLRequest requestWithURL:url1];
        NSMutableURLRequest *request = [httpClient requestWithMethod:@"GET"
                                                                path:@""
        ...
        AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request,                               NSHTTPURLResponse *response, id JSON) {
            ...
            NSLog(@"ステータス変更のget:%@",error);
            [SVProgressHUD showSuccessWithStatus:@"ステータス変更しました"];
            NSString *sessionId = [BSTutorialViewController sessions];
            NSString *urlString2 = [NSString stringWithFormat:@"%@/order_headers/get_order?session_id=%@&unique_key=%@",apiUrl,sessionId,importId];
            NSURL *url2 = [NSURL URLWithString:urlString2];
            AFHTTPClient *httpClient2 = [[AFHTTPClient alloc] initWithBaseURL:url2];
            //NSURLRequest *request = [NSURLRequest requestWithURL:url1];
            NSMutableURLRequest *request2 = [httpClient2 requestWithMethod:@"GET"
                                                                    path:@""
                                                              parameters:nil];
            AFJSONRequestOperation *operation2 = [AFJSONRequestOperation JSONRequestOperationWithRequest:request2 success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
                ...
                //NSDictionary *selectedOrderItem = ordersArray[orderNumber];
                //presentPayment = [ordersDict valueForKeyPath:@"OrderHeader.payment"];
                //NSString *status = [ordersDict valueForKeyPath:@"Order.status"];
