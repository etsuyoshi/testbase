...
- (void)setPayment:(id)JSON
{
    ...
    if ([[JSON valueForKeyPath:@"result.Shop.creditcart_payment"] intValue] == 1) {
        ...
        AFHTTPRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id results) {
            NSLog(@"payments/get_payment_service: %@", results);
            NSDictionary *error = results[@"error"];
            NSString *errormessage = error[@"message"];
            //NSString *errormessage1 = [JSON valueForKeyPath:@"result.Cart.1.error"];
            NSLog(@"Error message: %@", errormessage);
            ...
            NSString *creditcardPayment = [results valueForKeyPath:@"result.payment_service"];
