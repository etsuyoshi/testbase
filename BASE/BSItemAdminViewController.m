...
- (void)viewDidLoad
{
    ...
    if (!email) {
        ...
        dispatch_once(&token, ^{
            [BSTutorialViewController autoLogin:@"admin"];
        });
    }
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@/items/get_items?session_id=%@&=",apiUrl,session_id];
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        ...
        itemsArray = [JSON valueForKeyPath:@"result"];
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    NSString *urlString1 = [NSString stringWithFormat:@"%@/users/display_webview/get_shop?session_id=%@",apiUrl,[BSTutorialViewController sessions]];
    urlString1 = [urlString1 stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
    
    NSURL *url1 = [NSURL URLWithString:urlString1];
    AFHTTPClient *httpClient1 = [[AFHTTPClient alloc] initWithBaseURL:url1];
    //NSURLRequest *request = [NSURLRequest requestWithURL:url1];
    NSMutableURLRequest *request1 = [httpClient1 requestWithMethod:@"GET"
                                                              path:@""
                                                        parameters:nil];
    AFJSONRequestOperation *operation1 = [AFJSONRequestOperation JSONRequestOperationWithRequest:request1 success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        NSLog(@"users/display_webview: %@", JSON);
        
        int display = [JSON[@"result"][@"display"] intValue];
