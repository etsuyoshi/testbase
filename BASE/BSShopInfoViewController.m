...
- (void)viewDidLoad
{
    ...
    //グーグルアナリティクス
    self.trackedViewName = @"buyShopInfo";
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    NSString *urlString = [NSString stringWithFormat:@"%@/shops/get_shop?user_id=%@&version=%@",apiUrl,userId,version];
...
-(void)sendQuestion:(id)sender{
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:request success:^(NSURLRequest *request, NSHTTPURLResponse *response, id JSON) {
        NSLog(@"ログイン情報！: %@", JSON);
        NSArray *nameArray = [JSON valueForKeyPath:@"error.validations.ShopInquiry.name"];
        if (nameArray.count) {
            NSString *nameError = nameArray[0];
            [SVProgressHUD showErrorWithStatus:nameError];
            return ;
        }
        NSArray *mailArray = [JSON valueForKeyPath:@"error.validations.ShopInquiry.mail_address"];
        if (mailArray.count) {
            NSString *mail = mailArray[0];
            [SVProgressHUD showErrorWithStatus:mail];
            return ;
        }
        NSArray *inquiryArray = [JSON valueForKeyPath:@"error.validations.ShopInquiry.inquiry"];
